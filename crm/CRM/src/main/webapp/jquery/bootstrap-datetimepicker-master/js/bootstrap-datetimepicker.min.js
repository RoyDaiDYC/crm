...
function timeZoneAbbreviation(){var abbreviation,date,formattedStr,i,len,matchedStrings,ref,str;date=(new Date).toString();formattedStr=((ref=date.split('(')[1])!=null?ref.slice(0,-1):void 0)||date.split(' ');if(formattedStr instanceof Array){matchedStrings=[];for(i=0,len=formattedStr.length;i<len;i++){str=formattedStr[i];if(abbreviation=(ref=str.match(/\b[A-Z]+\b/))!=null?ref[0]:void 0){matchedStrings.push(abbreviation);}}
formattedStr=matchedStrings.pop();}
return formattedStr;};function UTCDate(){return new Date(Date.UTC.apply(Date,arguments));}
...
this.weekStart=this.weekStart%7;this.weekEnd=((this.weekStart+6)%7);this.onRenderDay=function(day){var render=(options.onRenderDay||function(){return[];})(day);if(typeof render=='string'){render=[render];}
var res=['day'];return res.concat((render?render:[]));};this.onRenderHour=function(hour){var render=(options.onRenderHour||function(){return[];})(hour);var res=['hour'];if(typeof render=='string'){render=[render];}
return res.concat((render?render:[]));};this.onRenderMinute=function(minute){var render=(options.onRenderMinute||function(){return[];})(minute);var res=['minute'];if(typeof render=='string'){render=[render];}
return res.concat((render?render:[]));};this.onRenderYear=function(year){var render=(options.onRenderYear||function(){return[];})(year);var res=['year'];if(typeof render=='string'){render=[render];}
return res.concat((render?render:[]));}
this.onRenderMonth=function(month){var render=(options.onRenderMonth||function(){return[];})(month);var res=['month'];if(typeof render=='string'){render=[render];}
return res.concat((render?render:[]));}
this.startDate=-Infinity;this.endDate=Infinity;this.datesDisabled=[];this.daysOfWeekDisabled=[];this.setStartDate(options.startDate||this.element.data('date-startdate'));this.setEndDate(options.endDate||this.element.data('date-enddate'));this.setDatesDisabled(options.datesDisabled||this.element.data('date-dates-disabled'));this.setDaysOfWeekDisabled(options.daysOfWeekDisabled||this.element.data('date-days-of-week-disabled'));this.setMinutesDisabled(options.minutesDisabled||this.element.data('date-minute-disabled'));this.setHoursDisabled(options.hoursDisabled||this.element.data('date-hour-disabled'));this.fillDow();this.fillMonths();this.update();this.showMode();if(this.isInline){this.show();}};Datetimepicker.prototype={constructor:Datetimepicker,_events:[],_attachEvents:function(){this._detachEvents();if(this.isInput){this._events=[[this.element,{focus:$.proxy(this.show,this),keyup:$.proxy(this.update,this),keydown:$.proxy(this.keydown,this)}]];}else if(this.component&&this.hasInput){this._events=[[this.element.find('input'),{focus:$.proxy(this.show,this),keyup:$.proxy(this.update,this),keydown:$.proxy(this.keydown,this)}],[this.component,{click:$.proxy(this.show,this)}]];if(this.componentReset){this._events.push([this.componentReset,{click:$.proxy(this.reset,this)}]);}}else if(this.element.is('div')){this.isInline=true;}else{this._events=[[this.element,{click:$.proxy(this.show,this)}]];}
...
html+='</tr>';this.picker.find('.datetimepicker-days thead').append(html);},fillMonths:function(){var html='',i=0;while(i<12){var classes=this.onRenderMonth(i+1)
...
this.picker.find('.datetimepicker-days tbody').empty().append(html.join(''));html=[];var txt='',meridian='',meridianOld='';var hoursDisabled=this.hoursDisabled||[];for(var i=0;i<24;i++){classes=this.onRenderHour(i+1);if(hoursDisabled.indexOf(i)!==-1){classes.push('disabled');}
...
this.picker.find('.datetimepicker-hours td').html(html.join(''));html=[];txt='',meridian='',meridianOld='';var minutesDisabled=this.minutesDisabled||[];for(var i=0;i<60;i+=this.minuteStep){if(minutesDisabled.indexOf(i)!==-1)continue;var actual=UTCDate(year,month,dayMonth,hours,i,0);classes=this.onRenderMinute(i+this.minuteStep);if(actual.valueOf()<this.startDate||actual.valueOf()>this.endDate){classes.push('disabled');}else if(Math.floor(minutes/this.minuteStep)==Math.floor(i/this.minuteStep)){classes.push('active');}
...
html='';year=parseInt(year/10,10)*10;var yearCont=this.setTitle('.datetimepicker-years',year+'-'+(year+9)).end().find('td');year-=1;for(var i=-1;i<11;i++){classes=this.onRenderYear(year);if(currentYear==year){classes.push('active');}
if(year<startYear||year>endYear){classes.push('disabled');}
if(i==-1||i==10){classes.push(old);}
